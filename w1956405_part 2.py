...
def printing_lists (alist,outcome):                                                  #used another user defined function to print the outcomes which were saved in lists

    for mark_set in alist:
        print(outcome,mark_set[0],",",mark_set[1],",",mark_set[2])
...
while True:                                                                                             #used another while loop to iterate multiple outcomes
    ...
    if total == 120 :                                               
        ...
        if response=="y":
            ...
            printing_lists(progress_list,"Progress -")                              #called the printing_lists user defined function 4 times for the 4 outcomes.
            printing_lists(trailer_list,"Progress (module trailer) -")
            printing_lists(retriever_list,"Module retriever -")
            printing_lists(exclude_list,"Exclude -")
